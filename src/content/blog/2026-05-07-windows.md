---
title: 'windows 11 — SYSTEM-context scheduled task writable by any user, world-writable named pipes with SID spoofing, and what the local privilege escalation surface looks like after a thorough sweep'
description: 'audited Windows 11 local privilege escalation surfaces: MareBackup scheduled task has GenericAll for BUILTIN\Users on a SYSTEM-context task definition, WiFiNetworkManagerTask named pipe trusts client-supplied SID at offset 0x14 on a world-writable pipe, Realtek HD Audio exposes a NULL-DACL named pipe to LocalSystem, KSecDD kernel IOCTL map recovered but sensitive operations gated to protected callers — plus the full surface scan across 20+ attack classes'
pubDate: '2026-05-07'
category: 'exploits'
---

I spent a few days scanning Windows 11 for local privilege escalation primitives. Not looking at kernel exploits or driver bugs — looking at the *configuration surface*: scheduled tasks, named pipes, services, COM objects, registry loadpoints, WMI subscriptions, device ACLs, AppX manifests. The kind of misconfigurations that give you SYSTEM from a medium-integrity unprivileged shell because someone set an ACL wrong or trusted a client-supplied value in a named pipe protocol.
...
|---------|--------|--------|
| Service ACLs | `sc sdshow` on all services — check WRITE_DAC, WRITE_OWNER, SERVICE_CHANGE_CONFIG | 0 writable services from medium-integrity |
